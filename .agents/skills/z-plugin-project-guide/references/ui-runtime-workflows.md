...
- ModStoryEvent Program uses only the first-level category selector; the former second-level type selector is hidden. `labId=0` is unclassified, and category changes assign a flat `EventProgramDataForm` Lab.
- New Event categories create a first-level `LabForm` path with `belong = nameof(EventProgramDataForm)` and remain listed even before a Program uses them.
...
- Event-edit Units omit empty argument slots and empty string arguments from their visual child list while retaining the syntax-tree positions and serialized call. Dynamic Event Program argument labels and separators are emitted only for visible arguments, with their original `paramN` indices.
...
- Event Program editing uses one first-level Lab list only. The former Type list and edit-window Type selector stay hidden; Program creation and category changes assign a first-level `EventProgramDataForm` Lab, and the Program list expands into the former Type column.
