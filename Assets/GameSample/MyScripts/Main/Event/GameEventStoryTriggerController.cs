    ...
    public void OnEvent(StoryLifeEvent evt)
    {
        switch (evt.type)
        {
            case StoryLifeEventType.FirstEnter:
                _super.Execute(EventProgramDataForm.DataByName.GetDv(GameManager.instance.curProgress.events.GetDv("onBeginEvent", EventTriggerForm.defaultData).evt,null),0,null);
                break;
        }
    }
}
