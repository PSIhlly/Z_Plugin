...
namespace Z_Map.Analysis
{
    ...
    public class NavigationController : Z_Controller<MapManager>
    {
        ...
        public void Build()
        {
            step = _super.data.mapUnitSize.y*1/10;
            InitMap(); 
            bfs = new Bfs(this);
        }
        ...
        public void InitMap()
        {
            ...
            Vector2[] offset = new Vector2[] { Vector2.right * 0.25f, Vector2.left * 0.25f, Vector2.up * 0.25f, Vector2.down * 0.25f };
                ...
                Vector3Int size = _super.data.size;
            ...
            for (int i = 0; i < size.x; i++)
                for (int j = 0; j < size.y; j++)
                    for (int k = 0; k < size.z; k++)
                    {
                        ...
                        navUnit.realPos = _super.data.maps[i, j, k].pos;
                        navUnit.pos = new Vector3Int(i,j,k);
                        navUnit.isNull = _super.data.maps[i, j, k].scale == Vector3.zero;
                    }
                    ...
                    for (int k = 0; k < size.z; k++)
                    {
                        ...
                        for (int l=0;l<4;l++)
                        {
                            ...
                            Vector2 p = new Vector2(tryDir[l].x*0.5f, tryDir[l].z * 0.5f);
                                ...
                                if (Math.Abs(link.GetYByPoint(-p)- map.GetYByPoint(p)) <=step)
                            {
