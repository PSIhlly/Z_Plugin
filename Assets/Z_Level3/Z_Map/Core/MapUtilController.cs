...
namespace Z_Map
{
    public class MapUtilController:Z_Controller<MapManager>
    {
        ...
        {
            var size = _super.dataCtrl.mainData.size;
            return (pos.x >= 0 && pos.x < size.x) && (pos.y >= 0 && pos.y < size.y) && (pos.z >= 0 && pos.z < size.z);
        }
        public bool InArea(Vector3 pos)
        {
            var size = _super.dataCtrl.mainData.size;
            return (pos.x >= -_super.dataCtrl.mainData.mapUnitSize.x/2 && pos.x < size.x + _super.dataCtrl.mainData.mapUnitSize.x / 2) && (pos.y >= -_super.dataCtrl.mainData.mapUnitSize.y / 2 && pos.y < size.y+ _super.dataCtrl.mainData.mapUnitSize.y / 2) && (pos.z >= -_super.dataCtrl.mainData.mapUnitSize.z / 2 && pos.z < size.z+_super.dataCtrl.mainData.mapUnitSize.z / 2);
        }
        ...
        {
            var size = _super.dataCtrl.mainData.size;
            var uSize = _super.dataCtrl.mainData.mapUnitSize;
