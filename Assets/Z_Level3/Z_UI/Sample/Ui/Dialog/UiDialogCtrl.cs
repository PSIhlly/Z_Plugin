...
namespace Ui.Dialog
{
    ...
    public partial class UiMainTextCtrl
    {
        ...
        public static float autoPlaydelay = 10f;

        public override void OnCreate()
        {
            view.btn_over.onClick.AddListener(() =>
            {
                Timer.CancelTimer(model.wordTimer);
        ...
        public void Display(string content = null)
        {
            ...
            var speed = Mathf.Max(0.1f, 1f / DialogManager.instance.settings.textDisplaySpeed);
            model.wordTimer = Timer.StartTimer(speed, Write, uiHolder);
        }
        public void DelayForOver(float delay)
        {
            Timer.CancelTimer(model.overTimer);
            model.overTimer = Timer.StartTimer(delay, () =>
            {
                Z_EventHelper.Invoke(new ClipPlayEvent()
                {
        ...
        public void OnEvent(ShowTypeEvent e)
        {
            switch (e.showType)
            {
                case ShowType.Normal:
                    view.go_normal.SetActive(true);
                    view.go_history.SetActive(false);
                    break;
                case ShowType.Hide:
                    view.go_normal.SetActive(false);
                    break;
